...
<form action="SearchTransaction.php?BPID=<?php echo $BPID;?>" method="post">
  <div class="form-row">
      ...
      <div class="form-group col-md-3">
        ...
        <select class="browser-default custom-select custom-select-lg mb-3" name="TransactionType">
...
if (is_null($CurrentMonth)) {
  echo "<script>alert('Not stated Month and year');</script>";
...
//echo "<script>window.location.href = 'NewTransaction2.php?BPID=$BPID';</script>";
  ...
  $TotalBudgetIncomeResult = mysqli_query($conn,$TotalBudgetIncomeSQL);
      ...
      if (is_null($TotalBudgetIncome) || is_null($BudgetMonth)) {
        echo "<script>alert('Not budget income information of the Month and year');</script>";
...
<table class="table text-white table-borderless">
  ...
  //    $BPID = $_GET['BPID'];
      ...
      var_dump($sql);
        ...
        $TransactionTotalExpenses = 0;
        $TransactionTotalIncome = 0;
        ...
        while ($rows = mysqli_fetch_array($result)) {
          ...
          if ($TransactionType == 'Income') {
            $TransactionTotalIncome += $TransactionAmount;
            $RunningIncomeBalance =  bcsub($BudgetIncome,$TransactionTotalIncome,2) ;
            //var_dump($BudgetIncome);
          }else{
            $TransactionTotalExpenses += $TransactionAmount;
