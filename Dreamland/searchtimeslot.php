  ...
  <div class="w3le-reg">
    ...
    <div class="mragilemain">
      ...
      <table class="PaymentDetails" >
        ...
        include 'conn.php';
          $searchkey= isset($_POST['tsearch'])?
          $_POST['tsearch']:'';

      if ($searchkey == NULL){
        echo "<script>alert('Please key in your search key first!');window.location.href = 'viewtimeslot.php';</script>";
      }
      else {
        $sql = "SELECT * FROM timeslotview WHERE MovieTitle Like '%".$searchkey."%' OR showdate Like '%".$searchkey."%' ORDER BY showdate DESC;";
        $result = mysqli_query($conn,$sql);
      //var_dump($sql);
      }
      if(mysqli_num_rows($result)<=0){
        echo "<script>alert('No result');</script>";
        die("<script>window.location.href = 'viewtimeslot.php';</script>");
      }
